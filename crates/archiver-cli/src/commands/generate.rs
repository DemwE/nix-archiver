...
pub fn cmd_generate(input: PathBuf, output: PathBuf, nixpkgs: Option<PathBuf>, db: ArchiverDb) -> Result<()> {
    ...
    //  1. --nixpkgs <path>  → builtins.fetchGit with local file:// (offline)
    //  2. sha256 in DB      → fetchTarball { url = ...; sha256 = "..."; } (pinned)
    //  3. fallback          → fetchTarball "<url>" (no sha256, not fully pinned)
    let source_expr = |commit: &str| -> String {
        ...
        if let Ok(Some(hash)) = db.get_tarball_hash(commit) {
            ...
            format!("fetchTarball \"{}\"", url)
        }
    };

    // Count commits that lack a tarball hash (for user warning)
    let missing_hashes: usize = if nixpkgs.is_none() {
        unique_commits.iter()
    ...
    } else if missing_hashes > 0 {
        eprintln!(
            "  {} {} commit{} without cached sha256. Run `nix-archiver prefetch-hashes` to pin fully.",
            "⚠".yellow(),
            missing_hashes,
            if missing_hashes == 1 { "" } else { "s" }
        );
    }
